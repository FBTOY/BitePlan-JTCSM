...
- **厨房档案**：人数、时间、水平、忌口。
- **储备管理**：按食材、配料/调料、工具分类维护；生成菜谱时可一键加入储备，并显示成功提示。
- **AI 生成菜谱**：支持 Kimi（Moonshot）和 Claude（Anthropic），在应用内配置 API Key。
- **偏好设置**：设置重量/容积单位规范，支持「只使用质量单位」选项；自定义家用单位（如 1 勺 = 5 克）；通过滑块量化甜、酸、苦、辣、咸口味偏好。
- **自动校准口味**：根据烹饪反馈自动调整口味偏好。
- **步骤压缩**：AI 生成后自动将短时间连续步骤合并，并提供下一步提示。
- **清单式跟做**：每个步骤配备 checklist，进度条展示每步标题，可随时查看全流程。
- **完成反馈**：收集评分、难度、口味与建议。
- **历史菜单**：查看过往烹饪记录，点击可查看完整菜谱、步骤与反馈详情。
